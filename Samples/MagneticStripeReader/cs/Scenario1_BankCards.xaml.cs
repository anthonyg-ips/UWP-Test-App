...
namespace MagneticStripeReaderSample
{
    ...
    public sealed partial class Scenario1_BankCards : Page
    {
        ...
        private async Task<bool> CreateDefaultMagneticStripeReaderObject()
        {
            if (_reader == null)
            {
                ...
                if (_reader == null)
                {
                    ...
                    LogBox.Text = "Magnetic Stripe Reader not found. Please connect a Magnetic Stripe Reader.";
                    return false;
                }
            }
            return true;
                ...
                if (_reader != null)
                {
                    ...
                    if (await ClaimReader())
                    {
                        if (_claimedReader != null)
                        {
                            ...
                            _claimedReader.ErrorOccurred += OnErrorOccurred;
                            ...
                            _claimedReader.DataEncryptionAlgorithm = MagneticStripeReaderEncryptionAlgorithms.None;
        ...
        async void OnBankCardDataReceived(object sender, MagneticStripeReaderBankCardDataReceivedEventArgs args)
        {
            ...
            {          
                rootPage.NotifyUser("Got data. AN Length:"+args.AccountNumber.Length, NotifyType.StatusMessage);
        ...
        async void OnVendorSpecificCardDataReceived(object sender, MagneticStripeReaderVendorSpecificCardDataReceivedEventArgs args)
        {
            // read the data and display
            await Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, () =>
            {
                rootPage.NotifyUser("Got Vendor Specific data", NotifyType.StatusMessage);
               
                if (args.Report == null)
                {
                    LogBox.Text += "Report was empty" + Environment.NewLine;
                }
                else
                {
                    LogBox.Text += "Properties:" + args.Report.Properties.Count + Environment.NewLine;
                    LogBox.Text += "Card Type:" + args.Report.CardType + Environment.NewLine;
                    if (args.Report.Track1 != null)
                    {
                        LogBox.Text+="Track 1 Data:" + args.Report.Track1.Data + Environment.NewLine;
                        LogBox.Text += "Track 1 EncryptedData" + args.Report.Track1.EncryptedData + Environment.NewLine;
                        LogBox.Text += "Track 1 DiscretionaryData" + args.Report.Track1.DiscretionaryData + Environment.NewLine;
                    }
                    if (args.Report.Track2 != null)
                    {
                        LogBox.Text += "Track 2 Data:" + args.Report.Track2.Data + Environment.NewLine;
                        LogBox.Text += "Track 2 EncryptedData" + args.Report.Track2.EncryptedData + Environment.NewLine;
                        LogBox.Text += "Track 2 DiscretionaryData" + args.Report.Track2.DiscretionaryData + Environment.NewLine;
                    }
                }
            });
        }

        async void OnErrorOccurred(object sender, MagneticStripeReaderErrorOccurredEventArgs args)
        {
            await Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, () =>
            {
                rootPage.NotifyUser("Error Occurred:" + args.ErrorData, NotifyType.StatusMessage);
                ErrorBox.Text = "Error Occurred" + Environment.NewLine;
                ErrorBox.Text = args.ErrorData.ToString();
                ErrorBox.Text += args.Track1Status;
            });
        }
